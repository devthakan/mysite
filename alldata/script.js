...
// ... (बाकी सभी ग्लोबल वेरिएबल्स पहले जैसे ही रहेंगे) ...


document.addEventListener('DOMContentLoaded', () => {
    // ... (यह पूरा सेक्शन पहले जैसा ही रहेगा) ...
});
        ...
        card.innerHTML = `
            <div class="card-header">
                ...
                <div class="card-header-text">
                    <h4>${item.name || 'N/A'}</h4>
                    <p>Aadhaar: ${item.aadhaar_number || 'N/A'}</p>
                </div>
            </div>
...
// === बाकी सभी फंक्शन्स पहले जैसे ही रहेंगे ===
// नीचे मैं पूरा कोड दे रहा हूँ ताकि कोई कन्फ्यूजन न हो।
// =========================================================================

const cameraModal = document.getElementById('camera-modal');const videoElement = document.getElementById('video');const canvasElement = document.getElementById('canvas');const cameraSelect = document.getElementById('camera-select');const captureBtn = document.getElementById('capture-btn');const closeModalBtn = document.getElementById('close-modal-btn');const cropModal = document.getElementById('crop-modal');const imageToCrop = document.getElementById('image-to-crop');const cropAndUploadBtn = document.getElementById('crop-upload-btn');const cropCancelBtn = document.getElementById('crop-cancel-btn');const cropUploadStatus = document.getElementById('crop-upload-status');
document.addEventListener('DOMContentLoaded', () => { const loginButton = document.getElementById('login-button'); const logoutButton = document.getElementById('logout-button'); const loginSection = document.getElementById('login-section'); const publicSection = document.getElementById('public-section'); const dashboardSection = document.getElementById('dashboard-section'); const publicSearchForm = document.getElementById('public-search-form'); const adminSearchForm = document.getElementById('admin-search-form'); const loginForm = document.getElementById('login-form'); const downloadTemplateBtn = document.getElementById('download-template-btn'); const uploadCsvBtn = document.getElementById('upload-csv-btn'); closeModalBtn.addEventListener('click', stopCamera); cameraSelect.addEventListener('change', startCamera); captureBtn.addEventListener('click', capturePhotoAndOpenCropper); cropCancelBtn.addEventListener('click', cancelCropping); cropAndUploadBtn.addEventListener('click', cropAndUploadImage); publicSearchForm.addEventListener('submit', handlePublicSearch); adminSearchForm.addEventListener('submit', handleAdminSearch); loginForm.addEventListener('submit', handleLogin); loginButton.addEventListener('click', () => { loginSection.style.display = loginSection.style.display === 'block' ? 'none' : 'block'; }); logoutButton.addEventListener('click', handleLogout); downloadTemplateBtn.addEventListener('click', downloadCSVTemplate); uploadCsvBtn.addEventListener('click', uploadCSV); checkUserSession(); });
function openCameraModal(aadhaarNumber) { currentAadhaarForPhoto = aadhaarNumber; cameraModal.style.display = 'flex'; populateCameraList().then(startCamera); }
function stopCamera() { if (currentStream) { currentStream.getTracks().forEach(track => track.stop()); } cameraModal.style.display = 'none'; }
function capturePhotoAndOpenCropper() { canvasElement.getContext('2d').drawImage(videoElement, 0, 0, canvasElement.width, canvasElement.height); const imageDataUrl = canvasElement.toDataURL('image/jpeg'); stopCamera(); cropModal.style.display = 'flex'; cropUploadStatus.innerHTML = ''; imageToCrop.src = imageDataUrl; if (cropper) { cropper.destroy(); } cropper = new Cropper(imageToCrop, { aspectRatio: 1 / 1, viewMode: 1, dragMode: 'move', background: false, autoCropArea: 0.8, }); }
function cancelCropping() { cropModal.style.display = 'none'; if (cropper) { cropper.destroy(); } }
async function cropAndUploadImage() { if (!cropper) return; cropUploadStatus.innerHTML = '<p>Cropping and compressing image...</p>'; cropAndUploadBtn.disabled = true; const croppedCanvas = cropper.getCroppedCanvas({ width: 400, height: 400, imageSmoothingQuality: 'high', }); croppedCanvas.toBlob(async (blob) => { if (!blob) { alert('Cropping failed.'); cropAndUploadBtn.disabled = false; return; } cropUploadStatus.innerHTML = '<p>Uploading to Supabase Storage...</p>'; const fileName = `${currentAadhaarForPhoto}_${Date.now()}.jpg`; const { data: uploadData, error: uploadError } = await supabaseClient.storage.from('farmer-photos').upload(fileName, blob, { upsert: false }); if (uploadError) { cropUploadStatus.innerHTML = `<p style="color: red;">Upload Failed: ${uploadError.message}</p>`; cropAndUploadBtn.disabled = false; return; } cropUploadStatus.innerHTML = '<p style="color: green;">Upload successful! Saving link...</p>'; const { data: urlData } = supabaseClient.storage.from('farmer-photos').getPublicUrl(uploadData.path); const newPhotoLink = urlData.publicUrl; const { error: updateError } = await supabaseClient.from('farmers').update({ photo_link: newPhotoLink }).eq('aadhaar_number', currentAadhaarForPhoto); if (updateError) { cropUploadStatus.innerHTML = `<p style="color: red;">Failed to save link: ${updateError.message}</p>`; } else { cropUploadStatus.innerHTML = '<p style="color: green;">All done!</p>'; document.getElementById(`photo-${currentAadhaarForPhoto}`).src = newPhotoLink; setTimeout(cancelCropping, 1500); } cropAndUploadBtn.disabled = false; }, 'image/jpeg', 0.8); }
...
function getGoogleDriveEmbedLink(driveLink) { if (!driveLink || driveLink.includes('supabase.co')) { return driveLink; } const match = driveLink.match(/\/d\/(.+?)(?:\/view|$)/); if (match && match[1]) { const fileId = match[1]; return `https://drive.google.com/uc?export=view&id=${fileId}`; } return driveLink; }
