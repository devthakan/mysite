...
async function captureAndUpload() {
    ...
    if (!isSnapshotTaken) {
        if (!video.videoWidth) { showToast('Video not ready. Try again.', 'error'); return; }
    ...
    croppedCanvas.toBlob(async (blob) => {
        ...
        if (updateError) { showToast(`Failed to save link: ${updateError.message}`, 'error'); btn.disabled = false;} 
        else {
            showToast('Photo updated successfully!', 'success');
            viewFarmerDetails(currentAadhaarForPhoto);
...
async function handleAdminSearch(e) { e.preventDefault(); const filters = []; const aadhaar = document.getElementById('admin-aadhaar-search').value.trim(); if (aadhaar) filters.push(`aadhaar_number.eq.${aadhaar}`); const account = document.getElementById('admin-account-search').value.trim(); if (account) filters.push(`account_number.eq.${account}`); const name = document.getElementById('admin-name-search').value.trim(); if (name) filters.push(`name.ilike.*${name}*`); const bl = document.getElementById('admin-bl-search').value.trim(); if (bl) filters.push(`bl_number.eq.${bl}`); const fatherName = document.getElementById('admin-father-search').value.trim(); if (fatherName) filters.push(`father_name.ilike.*${fatherName}*`); await loadAndDisplayFarmerTable(filters); }
...
async function checkUserSession() { const { data: { session } } = await supabaseClient.auth.getSession(); const loginButton = document.getElementById('login-button'); const logoutButton = document.getElementById('logout-button'); const loginSection = document.getElementById('login-section'); const publicSection = document.getElementById('public-section'); const dashboardSection = document.getElementById('dashboard-section'); if (session) { publicSection.style.display = 'none'; dashboardSection.style.display = 'block'; loginButton.style.display = 'none'; logoutButton.style.display = 'block'; loginSection.style.display = 'none'; showTableView(); loadAndDisplayFarmerTable(); } else { publicSection.style.display = 'block'; dashboardSection.style.display = 'none'; loginButton.style.display = 'block'; logoutButton.style.display = 'none'; } }
function showTableView() { document.getElementById('table-view').style.display = 'block'; document.getElementById('detail-view').style.display = 'none'; }
async function viewFarmerDetails(aadhaarNumber) { const detailViewContainer = document.getElementById('detail-view'); detailViewContainer.innerHTML = '<div>Loading Details...</div>'; const { data: item, error } = await supabaseClient.from('farmers').select('*').eq('aadhaar_number', aadhaarNumber).single(); if (error || !item) { showToast('Could not fetch farmer details.', 'error'); return; } document.getElementById('table-view').style.display = 'none'; detailViewContainer.style.display = 'block'; const photoLink = getGoogleDriveEmbedLink(item.photo_link); const imgSrc = photoLink || 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs='; const formattedExpireDate = item.application_expire ? new Date(item.application_expire).toISOString().split('T')[0] : ''; const card = document.createElement('div'); card.className = 'card'; card.id = `card-${item.aadhaar_number}`; card.innerHTML = `<button onclick="showTableView()" class="cancel-btn" style="float: right;"><i class="fas fa-arrow-left"></i> Back to List</button><div class="card-header"><img id="photo-${item.aadhaar_number}" src="${imgSrc}" alt="Farmer Photo" class="farmer-photo"><div class="card-header-text"><h4>${item.name || 'N/A'}</h4></div></div><div class="details-grid"><h5 class="group-title">Personal Details</h5><p><strong>Aadhaar Number:</strong><span class="readonly-field">${item.aadhaar_number || ''}</span></p><p><strong>Name:</strong><input type="text" id="name-${item.aadhaar_number}" value="${item.name || ''}"></p><p><strong>Father's Name:</strong><input type="text" id="father_name-${item.aadhaar_number}" value="${item.father_name || ''}"></p><p><strong>Gender:</strong><input type="text" id="gender-${item.aadhaar_number}" value="${item.gender || ''}"></p><p><strong>Age:</strong><input type="text" id="age-${item.aadhaar_number}" value="${item.age || ''}"></p><p><strong>Marriage Status:</strong><input type="text" id="marriage_status-${item.aadhaar_number}" value="${item.marriage_status || ''}"></p><p><strong>Category:</strong><input type="text" id="category-${item.aadhaar_number}" value="${item.category || ''}"></p><h5 class="group-title">Contact & Address</h5><p><strong>Mobile Number:</strong><input type="text" id="mobile_number-${item.aadhaar_number}" value="${item.mobile_number || ''}"></p><p><strong>WhatsApp Number:</strong><input type="text" id="whatsapp_number-${item.aadhaar_number}" value="${item.whatsapp_number || ''}"></p><p style="grid-column: 1 / -1;"><strong>Address:</strong><input type="text" id="address-${item.aadhaar_number}" value="${item.address || ''}"></p><h5 class="group-title">Financial & Application Details</h5><p><strong>BL Number:</strong><input type="text" id="bl_number-${item.aadhaar_number}" value="${item.bl_number || ''}"></p><p><strong>Account Number:</strong><input type="text" id="account_number-${item.aadhaar_number}" value="${item.account_number || ''}"></p><p><strong>Share Capital:</strong><input type="text" id="share_capital-${item.aadhaar_number}" value="${item.share_capital || ''}"></p><p><strong>Application Year:</strong><input type="text" id="application_year-${item.aadhaar_number}" value="${item.application_year || ''}"></p><p><strong>Application Expire:</strong><input type="date" id="application_expire-${item.aadhaar_number}" value="${formattedExpireDate}"></p><h5 class="group-title">Nominee Details</h5><p><strong>Nominee Name:</strong><input type="text" id="nominee_name-${item.aadhaar_number}" value="${item.nominee_name || ''}"></p><p><strong>Relation:</strong><input type="text" id="relation-${item.aadhaar_number}" value="${item.relation || ''}"></p><p><strong>Nominee Aadhaar:</strong><input type="text" id="nominee_aadhaar_number-${item.aadhaar_number}" value="${item.nominee_aadhaar_number || ''}"></p><div class="card-actions"><button onclick="updateRecord('${item.aadhaar_number}')"><i class="fas fa-save"></i> Save Changes</button><button onclick="openCameraModal('${item.aadhaar_number}')"><i class="fas fa-camera"></i> Update Photo</button><button class="delete-btn" onclick="deleteRecord('${item.aadhaar_number}')"><i class="fas fa-trash"></i> Delete</button></div></div>`; detailViewContainer.innerHTML = ''; detailViewContainer.appendChild(card); }
async function loadAndDisplayFarmerTable(filters = []) { const tableContainer = document.getElementById('farmer-table-container'); tableContainer.innerHTML = '<div>Loading farmer list...</div>'; let query = supabaseClient.from('farmers').select('aadhaar_number, name, father_name, mobile_number, bl_number'); if (filters.length > 0) { query = query.or(filters.join(',')); } query = query.order('name', { ascending: true }); const { data, error } = await query; if (error) { showToast(`Error fetching data: ${error.message}`, 'error'); tableContainer.innerHTML = ''; return; } if (!data || data.length === 0) { tableContainer.innerHTML = '<p>No matching records found.</p>'; return; } const table = document.createElement('table'); table.className = 'farmer-table'; table.innerHTML = `<thead><tr><th>Name</th><th>Father's Name</th><th>Aadhaar Number</th><th>Mobile Number</th><th>BL Number</th><th class="action-cell">Actions</th></tr></thead><tbody>${data.map(item => `<tr><td>${item.name || ''}</td><td>${item.father_name || ''}</td><td>${item.aadhaar_number || ''}</td><td>${item.mobile_number || ''}</td><td>${item.bl_number || ''}</td><td class="action-cell"><button class="view-btn" onclick="viewFarmerDetails('${item.aadhaar_number}')"><i class="fas fa-eye"></i> View</button></td></tr>`).join('')}</tbody>`; tableContainer.innerHTML = ''; tableContainer.appendChild(table); }
